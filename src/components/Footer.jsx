...
const Footer = () => {
  ...
  return (
    <footer className="border-t border-[#1F2622] bg-[#0B0F0E]">
      <div className="container-max mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-5 sm:gap-6">
          ...
          <div className="flex items-center gap-5">
            {links.map(({ icon: Icon, label, url }) => (
              <a
                ...
                aria-label={label}
                className="text-[#9CA3AF] hover:text-[#A3FF12] transition-colors duration-200 p-1"
          ...
          {/* Copyright */}
          <p className="text-xs text-[#9CA3AF]/50 text-center sm:text-right">
            © {new Date().getFullYear()} Giribalan K.
          </p>
        </div>
      </div>
    </footer>
